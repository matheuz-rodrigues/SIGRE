...
def create_period(period: PeriodCreate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
    return period_service.create(db, period)

@router.put("/{period_id}", response_model=PeriodOut)
def update_period(period_id: int, period: PeriodUpdate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
...
def delete_period(period_id: int, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
