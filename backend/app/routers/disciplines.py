...
from app.services.auth.rbac import require_role, require_role_exact, ROLE_USER, ROLE_ADMIN
...
def create_discipline(discipline: DisciplineCreate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
    return discipline_service.create(db, discipline)

@router.put("/{discipline_id}", response_model=DisciplineOut)
def update_discipline(discipline_id: int, discipline: DisciplineUpdate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
...
def delete_discipline(discipline_id: int, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
