...
def create_professor(professor: ProfessorCreate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
...
def update_professor(professor_id: int, professor: ProfessorUpdate, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
...
def delete_professor(professor_id: int, db: Session = Depends(get_db), _u=Depends(require_role_exact(ROLE_ADMIN))):
